...
public:
	...
	/** Receives FCDs from FcdService.
	 * Continuously receives FCDs generated by FcdService and enqueues them in the queue of the correct access category (VI).
	 * Before enqueuing, outdated DENMs are flushed from the queue.
	 * If a FCD is enqueued successfully, sendQueuedPackets() is called. Else, the queue is full and the FCD is dropped.
	 */
	void receiveFromFcd();
...
private:
	...
	CommunicationReceiver* mReceiverFromFcd;
	...
	boost::thread* mThreadReceiveFromFcd;
	boost::thread* mThreadReceiveFromHw;
	boost::thread* mThreadReceiveFromHw2;
